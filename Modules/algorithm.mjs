...
export async function PythonFeatureCalculation(args) {
  ...
  const pythonProcess = spawn('source venv/bin/activate && python3', [pythonScriptPath, ...args]);
...
export async function PythonCosineComparer(args)
{
  ...
  const pythonProcess = spawn('source venv/bin/activate && python3', [pythonCosinePath, ...args]);
